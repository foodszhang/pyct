...
fut_list = []
def read_circle(i):
    if not os.path.exists(f'../../data/123-3/{i}.tif'):
        return None
    img = cv2.imread(f'../../data/123-3/{i}.tif', -1)
...
ellipses =[]
for i in range(6):
    #(x, y), (a, b), theta = cv2.fitEllipse(np.array(ellipse_points[i])
    ellipse = cv2.fitEllipse(np.array(ellipse_points[i]))
    cv2.ellipse(zero_img, ellipse, (122,122,122), 1)
    ...
    A[i][1][1] = cy-b*cost
for t in A:
    cv2.circle(zero_img, (int(t[0][0]), int(t[0][1])), 3, (255,0,0), 1)
    cv2.circle(zero_img, (int(t[1][0]), int(t[1][1])), 3, (0,255,0), 1)
    cv2.circle(zero_img, (int(t[2][0]), int(t[2][1])), 3, (0,0,255), 1)
    cv2.circle(zero_img, (int(t[3][0]), int(t[3][1])), 3, (255,255,255), 1)
X =np.zeros(6)
Y = np.zeros(6)
for i in range(6):
...
print('center:', u0 ,v0)
print('偏移',u0 - (w+1)/2, (h+1)/2 -v0 )
print('SDD:',SDD*dpixel)
print('theta:', theta)

print('end?')
cv2.imwrite('ellipse.tif', zero_img)
